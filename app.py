...
app = Flask(__name__)
CORS(app)
app.secret_key = 'your-open-ai-key'

# Load the QA dataset
qa_dataset = [
    {
        "question":"tell me about jessup cellar",
        "answer":"Jessup Cellars has a casual and inviting atmosphere and was the first tasting room opened in Yountville in 2003. For mor information go to https://jessupcellars.com/"
    },
    {
    ...
    {
        "question":" what is the atmosphere like at jessup cellars?",
        "answer": "jessup cellars has a casual and inviting atmosphere, featuring a stunning art gallery and a patio with giant umbrellas. it also offers space for private groups and special accommodations for children(corpus)."
    },
    {
    "question": "where is jessup cellars located?",
    "answer": "jessup cellars is located in yountville, california, just a block north from the french laundry in the napa valley(corpus)."
    },
    {
    "question": "who are the founders of jessup cellars?",
    "answer": "jessup cellars was founded by dan and becky blue, roy and cheri eisiminger, vance and jana thompson, jim and kelly mazzo, kerry and cindi solomon, and eric and marleen donnenfeld(corpus)."
    },
    {
    "question": "what is the mission of jessup cellars?",
    "answer": "the mission of jessup cellars is to create and share life-enriching experiences for their members and guests, while also supporting the community through the arts(corpus)."
    },
    {
    "question": "what type of events does jessup cellars host?",
    "answer": "jessup cellars hosts interactive experiences focusing on the arts, including the art house sessions summer music series and art house short film series(corpus)."
    },
    {
    "question": "what are the benefits of jessup cellars wine club membership?",
    "answer": "members receive benefits such as exclusive access to limited-release wines, member-only events, a 15% discount on all current release orders, free tastings, and a $25 gift card if their guests become members(corpus)."
    },
    {
    "question": "how many wine club options does jessup cellars offer and what are they?",
    "answer": "jessup cellars offers three wine club options:1.	tasting club: 3 bottles four times per year 2.my jessup cellar 6: 6 bottles twice per year 3.	my jessup cellar 12: 12 bottles twice per year(corpus)."
    },
    {
    "question": "who is the consulting winemaker for jessup cellars?",
    "answer": "rob lloyd is the consulting winemaker for jessup cellars(corpus)."
    },
    {
    "question": "can you tell me more about rob lloyd’s background?",
    "answer": "rob lloyd started his career in the wine industry after graduating college and working in a tasting room. he earned his ms in viticulture & enology from uc davis and has worked for prestigious wineries like cakebread, stag’s leap wine cellars, and la crema. he began crafting wines for jessup cellars in 2009 and took the position of director of winemaking in 2010(corpus)."
    },
    {
    "question": "what is bernardo munoz's role at jessup cellars?",
    "answer": "bernardo munoz is a winemaker at jessup cellars, bringing his comprehensive grape-to-bottle knowledge to the team(corpus)."
    },
    {
    "question": "what types of white wines does jessup cellars produce?",
    "answer":" jessup cellars produces a napa valley chardonnay and an annual harvest of sauvignon blanc. the chardonnay is aged in american oak barrels, and the sauvignon blanc has a tropical nature distinct from typical valley-grown varieties(corpus)."
    },
    {
    "question": "what are the characteristics of jessup cellars' 2022 chardonnay?",
    "answer": "the 2022 chardonnay is well-balanced, aged in 40% new american and 60% neutral american oak, with a slightly creamy mouthfeel. it has an alcohol content of 14.8% and a ph of 3.4(corpus)."
    },
    {
    "question": "does jessup cellars produce any fortified wines?",
    "answer": "yes, jessup cellars produces fortified wines, including a 2013 zinfandel port and a 13th reflection tawny dessert wine. these wines are known for their bold flavors and traditional aging methods(corpus)."
    },
    {
    "question": "what is the solera method?",
    "answer": "the solera method is a traditional aging and blending process used for fortified wines, involving tiered stacking of barrels and fractional blending to maintain consistency and complexity in the final product(corpus)."
    },
    {
    "question": "who is the current resident artist at the jessup gallery?",
    "answer": "jermaine danté is the current resident artist at the jessup gallery. he is also a certified sommelier and captures the essence of wine country in his artwork(corpus)."
    },
    {
    "question": "what awards has jessup cellars won?",
    "answer": "jessup cellars has won awards such as cellarpass’ ‘friendliest & most knowledgeable staff’ for multiple years, napa valley life magazine’s ‘best wine club’, and yountville’s ‘business of the year’(corpus)."
    },
    {
    "question": "what are the characteristics of jessup cellars' 2019 graziella sangiovese blend?",
    "answer": "the 2019 graziella sangiovese blend has aromas and flavors of strawberry, red plum, raspberry, vanilla, and rose petal. it is bright with just the right amount of acidity and pairs well with red sauce dishes or spiced meats(corpus)."
    },
    {
    "question": "how should one pair the 2018 alexander valley cabernet sauvignon?",
    "answer": "the 2018 alexander valley cabernet sauvignon pairs well with quality cuts of beef or dishes with smoky, fatty, or charred qualities. it has a dry, tannic finish and is aged in 80% new french oak(corpus)."
    },
    {
    "question": "what is unique about the 2019 juel red wine from jessup cellars?",
    "answer": "the 2019 juel red wine is a blend of napa valley cabernet sauvignon, cabernet franc, merlot, petite sirah, petite verdot, and malbec. it is named in homage to the father of one of the owner families and is known for its balance and harmony(corpus)."
    },
    {
    "question": "how can one visit jessup cellars?",
    "answer": "jessup cellars is located at 6740 washington st. in yountville, ca. they are open from 10 am to 5:30 pm, seven days a week. reservations and information can be obtained by calling 707.944.5620 or visiting their website(corpus)."
    },
    {
        "question":"hello hi",
        "answer":"hie!! How may I help you?"
    }


]

def find_answer(question, qa_dataset):
    question = question.lower()
    best_match = None
    highest_score = 0
    
    for entry in qa_dataset:
        score = sum(1 for word in question.split() if word in entry['question'].lower())
...
def chat():
    ...
    # Try to find an answer in the custom dataset
    response = find_answer(user_message, qa_dataset)
    
    if response is None:
        # Fall back to GPT-4 if no answer is found in the custom dataset
        try:
            response = get_response_from_gpt4(conversation_history)
    ...
    # Save the conversation history back to the session
    session['history'] = conversation_history
    
    return jsonify({"response": response})

def get_response_from_gpt4(conversation_history):
